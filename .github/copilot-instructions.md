...
バックエンドは複数フレームワーク（FastAPI, Spring Boot 等）で同一 API を実装し、比較学習しています。
...
- `.github/instructions/backend-springboot.instructions.md` → `backend/springboot/**`
...
- SQLクエリの文字列結合・フォーマットによる組み立てを行わない（SQLAlchemy のパラメータバインドや Spring Data JPA のクエリメソッドを使う）
